...
import fundamentals
import market_telemetry
from entry_rules import rsi as _rsi, clean_bars, quote_quality, entry_verdict, validate_snapshot
...
def _download_one(kind, group, daily_period="1mo"):
    ...
    for attempt in range(RETRIES):
        try:
            data = yf.download(tickers=" ".join(group), group_by="ticker", auto_adjust=True, progress=False, threads=False, **kw)
            out = {}
            for sym in group:
                try:
...
def compute_metrics(intra, daily, now_et):
    ...
    m.update(quote_quality(intra, daily, now_et, MARKET_HOLIDAYS))
    intra, daily = clean_bars(intra), clean_bars(daily)
    if intra is not None and not intra.empty and daily is not None and not daily.empty:
        if _to_et(intra).index[-1].date() < daily.index[-1].date():
            intra = None
...
def compute_signal(daily, intra_m, ext_bias=0.0, event_risk=0.0, sector_dd=None):
    ...
    out={"action":"N/A","strength":0,"score":0,"rsi":None,"sma_state":None,"mom1m":None,"reasons":[],"note":None,
         ...
         "off_high_pct":None,"vs200_pct":None,"crash_flag":None,"buy_blockers":[],"history_days":0}
    rvol=None
    if intra_m:
        ...
        out.update({k: intra_m.get(k) for k in ("data_status", "data_reason", "quote_as_of", "history_as_of")})
    daily = clean_bars(daily)
    if daily is None or daily.empty: out["note"]="No data"; return entry_verdict(out)
    closes=daily["Close"].dropna()
    out["history_days"] = len(closes)
    price=_safe_float(out["price"]) or _safe_float(closes.iloc[-1]); out["price"]=round(price,2) if price else None
    if len(closes) < 50:
        out["note"] = f"Limited history ({len(closes)} daily closes); need 200 for new buys"
        return entry_verdict(out)
    ...
    if sma200 is None:
        out["buy_blockers"].append("fewer than 200 daily closes")
        out["note"] = f"Limited history ({len(closes)} daily closes); need 200 for new buys"
    ...
    if off_high<=-20:
        ...
        out["buy_blockers"].append("20% or more below the 52-week closing high")
    elif off_high<=-10:
    ...
    if sma200 is not None and price and price<sma200:
        out["buy_blockers"].append("below 200-day average")
    ...
    if sector_dd is not None and sector_dd<=-15:
        out["buy_blockers"].append("sector is 15% or more below its closing high")
    ...
    out["reasons"]=reasons; return entry_verdict(out)
...
def apply_lt_discipline(w):
    ...
    if tag == "strong" and score > 0:
        ...
        s.setdefault("buy_blockers", []).append("negative long-term drift")
    ...
    if s.get("buy_blockers") or s.get("crash_flag") in ("bear", "crash", "sector") or (s.get("vs200_pct") or 0) < 0:
...
def rank_picks(now_et, M, daily, market_bias, commod, event_risk, sec_health=None, top=5, watchlist=None):
    scored=[]
    sh=sec_health or {}
    canonical = {w["ticker"]: w["signal"] for w in (watchlist or []) if w.get("ticker")}
    for s in PICKS_UNIVERSE:
        sec=sector_of(s); ext=_clamp(market_bias+factors.sector_tilt(commod,sec),-1,1)
        sig = dict(canonical[s]) if s in canonical else compute_signal(daily.get(s),M(s),ext,event_risk,_sector_dd_for(s,sh))
        if sig.get("entry_action") != "BUY" or sig["price"] is None: continue
        scored.append({**sig, "symbol":s,"sector":sec})
...
def build_snapshot():
    ...
    picks = rank_picks(now_et, M, daily, market_bias, commod, event_risk, sec_health, watchlist=watchlist)
    if calib_state:
        _radar_h1 = ((crash_radar.load_state() or {}).get("h1") or {}).get("prob")
        for p in picks:                        # pooled-band fallback for non-watchlist names
            if p.get("conf_cal_pct") is not None: continue  # already adjusted on canonical watchlist
    ...
    try:
        top_calls_state = top_calls.compute(picks, watchlist, daily, as_of=now_et)
    except Exception as e:
    ...
    return {
        "updated_at": now_et.timestamp(), "updated_at_str": now_et.strftime("%Y-%m-%d %H:%M:%S ET"),
        ...
        "telemetry": market_telemetry.build(daily, intra, watchlist, sectors, now_et),
    }
...
def refresh_loop():
    ...
    while True:
        try:
            snap = validate_snapshot(build_snapshot())
            with _lock:
...
def api_data():
    with _lock:
        ...
        age = max(0, time.time() - data.get("updated_at", 0))
        max_age = 180 if data.get("session", {}).get("state") in ("pre", "regular", "post") else 900
        data["feed_status"] = {"stale": bool(_cache["error"]) or age > max_age,
                               "age_seconds": round(age), "error": _cache["error"]}
        return Response(json.dumps(data, allow_nan=False), mimetype="application/json", headers={"Cache-Control": "no-store"})
